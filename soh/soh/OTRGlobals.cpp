...
extern "C" __declspec(dllexport) void SOH_SetComboSharedItems(uint32_t mask) {
    gComboSharedMask = static_cast<int>(mask);
}
extern "C" __declspec(dllexport) uint32_t SOH_ReadComboSharedCVars(void) {
    uint32_t mask = 0;
...
extern "C" __declspec(dllexport) int SOH_GetSharedTier(int family) try {
...
extern "C" __declspec(dllexport) void SOH_RaiseSharedTier(int family, int tier) try {
    if (family < 0 || family >= ComboRando::SF_COUNT)
        return;
...
extern "C" __declspec(dllexport) void SOH_SetSharedChangedCb(void (*cb)(int, int)) {
    gComboSharedChanged = cb;
}
extern "C" void (*gComboSharedTick)(void) = nullptr;
extern "C" __declspec(dllexport) void SOH_SetSharedTickCb(void (*cb)(void)) {
    gComboSharedTick = cb;
}
