...
## Cross-entrance plumbing: parametric portal arrival (2026-07-05)

**Why:** groundwork for cross-game entrance shuffle (docs/ENTRANCE_RANDO_PREP.md §4). Both combo
resume shortcuts previously hardcoded the arrival point (Market outside Mask Shop / South Clock
Town); cross-entrances need the launcher to choose it per switch. An arrival latch
(`gComboCrossArrival`) stops combo-driven arrivals from being read as new portal triggers by the
future seed-driven door hooks. Which doors map where arrives later with the portal table.
