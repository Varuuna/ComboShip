...
// between many grottos, so they can't be reversed cleanly and belong to their own pool.
...
std::vector<EntrancePair> GetOverworldEntrances() {
    ...
    std::set<s32> paired; // dedupes the mirror direction of each connection
    ...
    for (const auto& [destination, returnEntrance] : edges) {
        ...
        if (paired.count(destination) || paired.count(returnEntrance)) {
            continue;
        }
        overworldEntrances.push_back({ returnEntrance, destination });
        paired.insert(destination);
        paired.insert(returnEntrance);
    }
...
std::set<RandoRegionId> ReachableRegions() {
    ...
    while (!frontier.empty()) {
        ...
        if (regionId == RR_MAX) { // only the save warp; owl warps need on-foot activation first
            frontier.push_back(Rando::Logic::GetRegionIdFromEntrance(ENTRANCE(SOUTH_CLOCK_TOWN, 0)));
            continue;
        }
        ...
        for (auto& [exitId, regionExit] : Rando::Logic::Regions[regionId].exits) {
            // Grotto exits always return to their vanilla spawn (see the OnPlayDestroy hook), so don't
            // follow a shuffled destination for them.
            s32 lookupExit = IsGrottoEntrance(regionExit.returnEntrance) ? exitId : GetShuffledEntrance(exitId);
            frontier.push_back(Rando::Logic::GetRegionIdFromEntrance(lookupExit));
        }
    }
    return reachable;
}

// True if the current shuffle map leaves every region reachable from the start.
...
void ApplyEntranceShuffle(const std::vector<EntrancePoolType>& poolsToShuffle) {
...
void ShuffleEntrances() {
    ...
    if (RANDO_SAVE_OPTIONS[RO_SHUFFLE_ENTRANCES_INTERIORS]) {
        poolsToShuffle.push_back(POOL_INTERIOR);
    }
    if (RANDO_SAVE_OPTIONS[RO_SHUFFLE_ENTRANCES_DUNGEONS]) {
        poolsToShuffle.push_back(POOL_DUNGEON);
    }
    if (RANDO_SAVE_OPTIONS[RO_SHUFFLE_ENTRANCES_OVERWORLD]) {
        poolsToShuffle.push_back(POOL_OVERWORLD);
    }
    ...
    for (uint32_t attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        ApplyEntranceShuffle(poolsToShuffle);
