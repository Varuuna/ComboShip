...
class Fast3dGui : public Ship::Gui {
  public:
    ...
    // ComboShip: invalidate the Fast3D renderer backend's font texture so the next frame rebuilds
    // the atlas (needed after a game adds fonts to the shared atlas post-init). See Ship::Gui::RebuildFontTexture.
