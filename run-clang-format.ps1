...
# ComboShip: format BOTH games' port trees (soh/ + mm/), mirroring run-clang-format.sh (the CI gate).
# Rule (same for both games): format .c/.cpp everywhere; format .h/.hpp EXCEPT the decomp header dirs
# (src/, include/) since those are formatted upstream as a no-op; skip autogenerated assets/ (and build/).
# clang-format picks each file's nearest .clang-format (soh/.clang-format vs mm/.clang-format) automatically.
$files = Get-ChildItem -Path "$basePath\soh", "$basePath\mm" -Recurse -File `
    | Where-Object { ($_.Extension -eq '.c' -or $_.Extension -eq '.cpp' -or `
                     ...
                     (-not ($_.FullName -like "*\soh\assets\*" -or $_.FullName -like "*\soh\build\*" -or `
                            $_.FullName -like "*\mm\assets\*"  -or $_.FullName -like "*\mm\build\*")) }
