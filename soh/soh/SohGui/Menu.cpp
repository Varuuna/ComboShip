...
void Menu::DrawContent(const std::set<std::string>& skipPaths) {
    ...
    // depends on: refresh disable evaluations and the race-disable flag.
    for (auto& [reason, info] : disabledMap) {
        info.active = info.evaluation(info);
    }
    raceDisableActive = CVarGetInteger(CVAR_SETTING("DisableChanges"), 0);
    ...
    // ComboShip: run any registered per-section update funcs (e.g. resolution mirrors) before
    // drawing widgets, exactly as DrawElement does.
