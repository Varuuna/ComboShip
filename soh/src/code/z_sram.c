...
#ifdef COMBO_BUILD
    // ComboShip Inc2 (Task 3): run the combo generator BEFORE reading questType so that
    // SOH_ApplyRandoPlacements + SetSeedGenerated(true) are complete before
    // Randomizer_InitSaveFile is invoked. Also force questType to QUEST_RANDOMIZER
    // because ComboShip is rando-only; the real seed is generated by the callback.
