...
class Context {
  public:
    ...
    /**
     * @brief Swaps the active ResourceManager without destroying the previous one (caller retains it).
     *
     * ComboShip: lets each game keep its own ResourceManager resident across a game switch (OOT<->MM)
     * so archives/resource caches stay loaded and data other globals point into is not freed.
     */
